...
public class ThreadedServer {
    ...
    public static void main(String args[]) {
        ...
        List<ThreadedConnectionHandler> con = new ArrayList<ThreadedConnectionHandler>();
        ...
        while (listening) // almost infinite loop - loop once for each client request
        {
            ...
            con.add(new ThreadedConnectionHandler(clientSocket, gui));
            con.get(con.size() - 1).start();

            // for(ThreadedConnectionHandler temp: con){

            // robots.add(temp.getRobots());
            // }
            // gui.update();
